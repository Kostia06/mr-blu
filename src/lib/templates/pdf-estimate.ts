 ...
 * Implements PDFTemplate<EstimateData> for server-side PDF generation via Puppeteer.
...
<div style="padding: 48px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
	<h1 style="font-size: 32px; font-weight: 700; color: #0066FF; margin: 0 0 4px;">ESTIMATE</h1>
	...
	<div style="display: flex; gap: 150px; padding: 14px 24px; background: #f5f5f5; border-radius: 6px; margin-bottom: 32px;">
		<div>
			<span style="font-size: 8px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280;">Estimate Date</span><br>
			<span style="font-size: 11px; font-weight: 600;">${formatDate(data.documentDate)}</span>
		</div>
		${data.validUntil ? `<div>
			<span style="font-size: 8px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280;">Valid Until</span><br>
			<span style="font-size: 11px; font-weight: 600;">${formatDate(data.validUntil)}</span>
		</div>` : ''}
	</div>
	...
	<div style="width: 220px; margin-left: auto;">
		...
		<div style="display: flex; justify-content: space-between; padding-top: 10px; margin-top: 8px; border-top: 2px solid #1a1a1a;">
			<span style="font-size: 14px; font-weight: 700;">Estimated Total</span>
	...
	<div style="margin-top: 24px;">
		<p style="font-size: 8px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280; margin-bottom: 6px;">Terms &amp; Conditions</p>
		<p style="font-size: 10px; color: #6b7280; line-height: 1.5;">${escapeHtml(data.terms)}</p>
	</div>` : ''}
...
export function mapSourceToEstimate(source: DocumentSourceData): EstimateData {
	return {
