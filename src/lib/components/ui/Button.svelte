<script lang="ts">
	...
	const computedClass = $derived(
		`${baseClasses} ${variants[variant]} ${sizes[size]} ${pulse ? 'pulse-glow' : ''} ${className}`
	);
</script>

{#if href && !disabled}
	<a {href} class={computedClass} use:ripple>
		{#if loading}
			<svg
...
{:else}
	<button class={computedClass} {disabled} {onclick} use:ripple>
		{#if loading}
			<svg
				class="animate-spin h-5 w-5"
				xmlns="http://www.w3.org/2000/svg"
