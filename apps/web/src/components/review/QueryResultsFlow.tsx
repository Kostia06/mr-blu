...
function getDocIconClasses(type: string): string {
  ...
  if (type === 'estimate') return 'bg-purple-500/10 text-purple-400';
...
function getMatchLabel(similarity: number, t: (key: string) => string): { label: string; classes: string } {
  if (similarity >= 0.8) {
    return { label: t('review.highMatch'), classes: 'bg-emerald-500/15 text-emerald-400' };
  }
  if (similarity >= 0.6) {
    return { label: t('review.goodMatch'), classes: 'bg-sky-500/15 text-sky-400' };
  }
  return { label: t('review.possibleMatch'), classes: 'bg-[var(--gray-100)] text-[var(--gray-500)]' };
}
  ...
  return (
    <div class="flex-1 px-[var(--page-padding-x)] max-w-[var(--page-max-width)] mx-auto w-full flex flex-col gap-[var(--section-gap)]">
      ...
      {!isLoading && queryResult && (
        <>
          ...
          {queryResult.summary && (
            <>
              ...
              {Object.keys(queryResult.summary.byStatus).length > 0 && (
                <div class="mt-4">
                  <h4 class="text-xs font-semibold text-[var(--gray-500)] uppercase tracking-wider mb-2.5 mt-0">
                    {t('review.byStatus')}
                  </h4>
          ...
          {queryResult.documents && queryResult.documents.length > 0 ? (
            <div class="mt-5">
              <h4 class="text-sm font-semibold text-[var(--gray-600)] mb-3 mt-0">
          ...
          {queryResult?.suggestions?.alternatives && queryResult.suggestions.alternatives.length > 0 && (
            <div class="mt-5 p-4 bg-amber-500/[0.08] border border-amber-500/20 rounded-[14px]">
              <h4 class="flex items-center gap-2 text-sm font-semibold text-amber-400 mb-1.5 mt-0">
                <Search size={16} />
                {queryResult.documents?.length ? t('review.similarClients') : t('review.didYouMean')}
              </h4>
              <p class="text-[13px] text-[var(--gray-500)] mb-3.5 mt-0">
              ...
              <div class="flex flex-col gap-2">
                {queryResult.suggestions.alternatives.map((suggestion) => {
                  ...
                  return (
                    <button
                      key={suggestion.id}
                      class="flex items-center justify-between px-3.5 py-3 border border-[var(--gray-200)] rounded-[10px] transition-all duration-200 text-left cursor-pointer bg-transparent hover:bg-amber-500/15 hover:border-amber-500/30 hover:-translate-y-px"
      ...
      <div class="flex gap-3 mt-6 pt-5 border-t border-[var(--gray-200)]">
        ...
        <button
          class="flex-1 flex items-center justify-center gap-2 px-5 py-3.5 rounded-xl text-sm font-semibold cursor-pointer transition-all duration-200 border-none bg-gradient-to-br from-[#0066ff] to-[#0052cc] text-white shadow-[0_4px_12px_rgba(0,102,255,0.25)]"
