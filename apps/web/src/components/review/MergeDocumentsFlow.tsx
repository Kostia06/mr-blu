  ...
  return (
    <div class="flex-1 px-[var(--page-padding-x)] max-w-[var(--page-max-width)] mx-auto w-full flex flex-col gap-[var(--section-gap)]">
      <div class="bg-sky-500/5 border border-sky-500/20 rounded-[var(--radius-card)] p-[var(--space-5)]">
        <div class="flex items-center gap-[var(--space-2)] mb-[var(--space-2-5)] text-[#38bdf8] text-[var(--text-sm)] font-[var(--font-medium)]">
      ...
      <div class="mt-5">
        ...
        {mergeSourceSelections.map((sel, index) => (
          <div key={index} class="mb-6 p-4 border border-[var(--gray-200)] rounded-[14px]">
            <h4 class="flex items-center gap-2.5 text-sm font-semibold text-[var(--gray-900)] mb-3.5 mt-0">
              <span class="w-6 h-6 flex items-center justify-center bg-sky-500/20 text-[#38bdf8] rounded-full text-xs font-bold">
                {index + 1}
              </span>
                ...
                {sel.documents.map((doc) => {
                  ...
                  return (
                    <button
                      key={doc.id}
                      class={cn(
                        'flex items-center gap-3 px-3.5 py-3 bg-[var(--white)] border border-[var(--gray-200)] rounded-xl text-left transition-all duration-200 cursor-pointer',
                    ...
                    >
                      ...
                      <div class="flex-1 flex flex-col gap-0.5 min-w-0">
                        ...
                        <span class="text-[11px] text-[var(--gray-500)]">{formatQueryDate(doc.date)}</span>
                      </div>
                      <div class="text-sm font-semibold text-[var(--data-green)]">
                        {formatQueryAmount(doc.amount)}
                      </div>
                      {isSelected && <Check size={16} class="text-[#38bdf8] shrink-0" />}
                    </button>
                  );
                })}
              </div>
