	...
	return (
		<main className="min-h-screen bg-transparent">
			<SettingsPageHeader
				...
				right={
					<button
						className="w-10 h-10 flex items-center justify-center bg-white/50 backdrop-blur-[12px] border-none rounded-[14px] text-[var(--blu-primary,#0066ff)] cursor-pointer"
			...
			<div className="px-5 pb-[100px] max-w-[600px] w-full mx-auto">
				...
				{/* Create form */}
				{showCreate && (
					<div className="bg-white/60 backdrop-blur-[12px] border-[1.5px] border-[var(--blu-primary,#0066ff)] rounded-2xl p-4 mb-3">
						<div className="flex items-center justify-between mb-3">
							<span className="text-sm font-bold text-[var(--blu-primary,#0066ff)]">
								{t('clients.newClient')}
							</span>
							<div className="flex gap-1">
								...
								<button className="flex items-center justify-center w-8 h-8 border-none bg-transparent rounded-lg cursor-pointer p-0" onClick={cancelCreate} aria-label="Cancel">
						...
						<div className="flex flex-col gap-2">
							<input
								...
								className="w-full border-[1.5px] border-[var(--blu-primary,#0066ff)] bg-white/80 rounded-[10px] px-2.5 py-2 text-[15px] font-semibold text-[var(--gray-900,#0f172a)] outline-none font-[inherit] mb-2 box-border"
								autoFocus
								onKeyDown={(e) => {
							...
							<div className="flex items-center gap-2">
								...
								<textarea
									...
									className="flex-1 border border-[var(--gray-200,#e2e8f0)] bg-white/80 rounded-[10px] px-2.5 py-1.5 text-[13px] text-[var(--gray-900,#0f172a)] outline-none font-[inherit] resize-y min-h-9"
									rows={2}
								/>
							</div>
						</div>
				...
				{loading ? (
					...
					<div className="flex flex-col items-center justify-center gap-3 px-5 py-[60px] text-center">
						<Users size={40} className="text-[var(--gray-300,#cbd5e1)]" />
					...
					<div className="flex flex-col gap-2.5">
						{filtered.map((entry) => (
							<div
								key={entry.id}
								className="bg-white/50 backdrop-blur-[12px] border border-white/50 rounded-2xl p-4 cursor-pointer"
							...
							>
								<div className="flex items-center justify-between mb-2.5">
									<div className="flex items-center gap-2.5 min-w-0 flex-1">
										<div className="w-9 h-9 flex items-center justify-center bg-[rgba(0,102,255,0.1)] rounded-full text-[15px] font-bold text-[var(--blu-primary,#0066ff)] shrink-0">
										...
										{editingId === entry.id ? (
											...
											<span className="text-[15px] font-semibold text-[var(--gray-900,#0f172a)] whitespace-nowrap overflow-hidden text-ellipsis">
												{entry.name}
											</span>
										)}
									</div>
									<div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
										{editingId === entry.id ? (
											...
											<>
												...
												<ChevronRight size={16} className="text-[var(--gray-300,#cbd5e1)]" />
											</>
										)}
									</div>
								</div>
										...
										<div className="flex flex-col gap-1.5">
											{entry.email && (
												<div className="flex items-center gap-2">
													...
													<span className="text-[13px] text-[var(--gray-500,#64748b)] whitespace-nowrap overflow-hidden text-ellipsis">
														{entry.email}
													</span>
												</div>
											)}
											...
											{entry.notes && (
												<div className="flex items-center gap-2">
													...
													<span className="text-[13px] text-[var(--gray-400,#94a3b8)] italic whitespace-pre-wrap break-words">
														{entry.notes}
													</span>
												</div>
											)}
