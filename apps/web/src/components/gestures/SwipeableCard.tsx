...
export function SwipeableCard({
  ...
  return (
    <div class="relative overflow-hidden rounded-[var(--radius-button,14px)]" role="group" aria-label="Swipeable card">
      {/* Left action (delete) */}
      <div
        class="absolute top-0 bottom-0 left-0 w-[120px] flex flex-col items-center justify-center gap-1.5 pointer-events-none rounded-l-[var(--radius-button,14px)]"
        style={{
          background: 'linear-gradient(90deg, rgba(239, 68, 68, 0.95) 0%, rgba(239, 68, 68, 0.8) 100%)',
          opacity: leftOpacity,
        }}
      >
        <div class="w-11 h-11 flex items-center justify-center rounded-full bg-white/20">
          <Trash2 size={24} class="text-white" />
        </div>
        <span class="text-xs font-semibold text-white uppercase tracking-[0.05em]">{t('swipe.delete')}</span>
      </div>

      {/* Right action (send) */}
      <div
        class={cn(
          'absolute top-0 bottom-0 right-0 w-[120px] flex flex-col items-center justify-center gap-1.5 pointer-events-none rounded-r-[var(--radius-button,14px)]',
        )}
        style={{
      ...
      >
        ...
        <span class="text-xs font-semibold text-white uppercase tracking-[0.05em]">
