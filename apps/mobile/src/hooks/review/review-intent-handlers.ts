import type { ParsedData, QueryData, IntentType } from '@/lib/review/review-types';

const TAX_RATE_DECIMAL_THRESHOLD = 1;
...
export function handleDocumentAction(
  ...
  if (newData.items) {
    for (const item of newData.items) {
      ...
      if (!item.measurementType) {
        item.measurementType = inferMeasurementType(item);
      }
    }
  }
  ...
  if (newData.actions) {
    newData.actions = newData.actions.map((action, i) => ({
      ...action,
      id: action.id || `action-${Date.now()}-${i}`,
...
function inferMeasurementType(item: { dimensions?: any; unit?: string; quantity?: number }) {
  if (item.dimensions?.width && item.dimensions?.length) {
    return 'sqft';
  }
  if (item.unit) {
    const u = item.unit.toLowerCase();
    if (u === 'sqft' || u === 'sq ft' || u === 'sf') return 'sqft';
    if (u === 'ft' || u === 'linear_ft' || u === 'lf') return 'linear_ft';
    if (u === 'hr' || u === 'hour' || u === 'hours') return 'hour';
    if (item.quantity === 1 && (u === 'unit' || u === 'job' || u === 'ea')) return 'service';
  }
  if (item.quantity === 1) return 'service';
  return undefined;
}
